...
class Controller:
	...
	def __init__(self, port="/dev/ttyACM0"):
		...
		self.L_FB_CURRENT = 0x20AD
		self.R_FB_CURRENT = 0x20AE
		
		
		## Emergency stop
		self.ESTOP = 0x2021
		self.QUICK_STOP_REG = 0x2011
	...
	def trigger_software_estop(self):
		"""
		Triggers an immediate software emergency stop over RS485.
		How the motor stops depends on register 2011h.
		"""
		result = self.client.write_register(self.CONTROL_REG, self.EMER_STOP, unit=self.ID)
		return result
	def set_io_estop_mode(self, input1_mode, input2_mode=0):
		"""
		Configures the hardware I/O emergency stop behavior (Register 0x2021).
		Modes:
		0: Lock shaft (Motor stops with holding force)
		1: Damping mode (Short-circuits power cables for resistance)
		2: Release shaft (Turns off PWM, free-running)
		"""
		# The manual states: Low 8 bits = INPUT1, High 8 bits = INPUT2
		combined_mode = (input2_mode << 8) | input1_mode
		
		result = self.client.write_register(self.ESTOP, combined_mode, unit=self.ID)
		return result
		
	def set_quick_stop_mode(self, mode=5):
		"""
		Configures how the driver processes a quick stop command (Register 0x2011).
		Modes:
		5: Stop normally (with deceleration time)
		6: Quick stop (with emergency deceleration time)
		7: Quick stop (without deceleration time, inertia stop)
		"""
		
		result = self.client.write_register(self.QUICK_STOP_REG, mode, unit=self.ID)
		return result
	...
	def get_current(self):
		# Read 2 sequential registers starting at the Left Current register
